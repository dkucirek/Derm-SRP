...
result_wide <- result_closest_all %>%
  as_tibble() %>%
  pivot_wider(
    id_cols = c(mrn_deid, enc_date, appt_dept, icd10_dx, icd_diagnosis),
    names_from = phq_field,
    values_from = c(phq_value, phq_value_num, phq_date),
    names_glue = "{.value}_{phq_field}"
  )
...
```
doing some phq2 checking-- freq of total score listed, freq of manual score, freq of ps/neg listed and freq of ps/neg possible to be calculated 
